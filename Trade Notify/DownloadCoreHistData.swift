...
func setHistDataUrl (managedContext: NSManagedObjectContext, coreStocks: [NSManagedObject]) -> [String] {
    
    var histData = [NSManagedObject]()
    ...
    for i in 0 ..< coreStocks.count {
        ...
        if let stockNameTemp = (coreStocks[i].valueForKey("name") as? String) {
            stockName = stockNameTemp
        } else {
            print("stock not found in setHistDataFunction")
        }
        
        //set temporary array for stock linked data anaysis
        
        histData = loadStockHistData(managedContext, stockName: stockName)!
        print("setHistDataUrl: histData.count for \(stockName):", histData.count)
        
        //make sure historical data is up to date
        if histData.count > 0 {
            lastDownloadDate = histData[0].valueForKey("date") as! NSDate
...
func getPricesURL(stocks: [NSManagedObject])-> [String] {
    ...
    for i in 0 ..< stocks.count {
        ...
        stockName = stocks[i].valueForKey("name") as! String
        url = String("http://finance.yahoo.com/d/quotes.csv?s=\(stockName)&f=l1sabc1p2")
        urls.append(url)
